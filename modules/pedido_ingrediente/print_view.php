...
$id_pedido = isset($_GET['id_pedido_ingrediente']) ? $_GET['id_pedido_ingrediente'] : '';
...
if ($start_date && $end_date) {
    $query = mysqli_query($mysqli, "SELECT pc.*, s.sucursal ,u.username
    FROM pedido_ingrediente pc
...
if ($id_pedido) {
    ...
    FROM pedido_ingrediente pc
    JOIN usuarios u ON pc.id_user = u.id_user
    JOIN sucursal s ON u.id_sucursal = s.id_sucursal WHERE id_pedido_ingrediente = $id_pedido")
        or die('Error: ' . mysqli_error($mysqli));
    ...
    // Detalle de compra
    $detalle_compra_query = mysqli_query($mysqli, "SELECT * FROM v_detalle_pedido_ingrediente WHERE id_pedido_ingrediente = $id_pedido")
        or die('Error: ' . mysqli_error($mysqli));
...
<body>
    <div class="content">
                ...
                <tbody>
                    <?php if (count($pedidos) > 0): ?>
                        <?php foreach ($pedidos as $pedido): ?>
                            <tr>
                                <td><?php echo htmlspecialchars($pedido['id_pedido_ingrediente']); ?></td>
                    ...
                    <?php else: ?>
                        <tr>
                            <td colspan="5">No se encontraron pedidos para el rango de fechas seleccionado.</td>
                        </tr>
                    <?php endif; ?>
                </tbody>
            </table>
        ...
        <?php if ($id_pedido && isset($_GET['act']) && $_GET['act'] == 'imprimir'): ?>
            <h3>Detalle del Pedido N° <?php echo htmlspecialchars($cabecera_compra['id_pedido_ingrediente']); ?></h3>
            <table>
                <tr>
                    <th>Código</th>
                    <td><?php echo htmlspecialchars($cabecera_compra['id_pedido_ingrediente']); ?></td>
                </tr>
                <tr>
                    <th>Sucursal</th>
                ...
                <tr>
                    <th>Usuario</th>
                    <td><?php echo htmlspecialchars($cabecera_compra['username']); ?></td>
                </tr>
                <tr>
                    <th>Estado</th>
                ...
                <tbody>
                    <?php if (count($detalle_compra) > 0): ?>
                        <?php foreach ($detalle_compra as $detalle): ?>
                            <tr>
                                <td><?php echo htmlspecialchars($detalle['id_pedido_ingrediente']); ?></td>
