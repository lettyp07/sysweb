...
if (empty($_SESSION['username']) && empty($_SESSION['password'])){
	...
	if ($_GET['act'] == 'on' && isset($_GET['id'])) {
		$id_user = mysqli_real_escape_string($mysqli, $_GET['id']);
		
		// Cambia el estado a "activo" y restablece los intentos fallidos a 0
		$query = "UPDATE usuarios SET status='activo', intentos_fallidos=0 WHERE id_user='$id_user'";
		
		if (mysqli_query($mysqli, $query)) {
			header("location: ../../main.php?module=user&alert=3"); // Redirige con un mensaje de éxito
		} else {
