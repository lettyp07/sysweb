...
$id_pedido = isset($_GET['id_control_calidad']) ? $_GET['id_control_calidad'] : '';
...
if ($id_pedido) {
    // Cabecera de compra
    $cabecera_compra_query = mysqli_query($mysqli, "SELECT pc.*, s.sucursal,u.username
    FROM control_calidad pc
    JOIN usuarios u ON pc.id_user = u.id_user
    JOIN sucursal s ON u.id_sucursal = s.id_sucursal WHERE id_control_calidad = $id_pedido")
        or die('Error: ' . mysqli_error($mysqli));
    ...
    // Detalle de compra
    $detalle_compra_query = mysqli_query($mysqli, "SELECT d.*, t.t_producto, p.id_producto, p.descrip, u.u_descrip FROM detalle_control_calidad d
    JOIN producto p
    ...
    WHERE id_control_calidad = $id_pedido")
        or die('Error: ' . mysqli_error($mysqli));
...
<head>
    <meta charset="UTF-8">
    <title>Reporte de Control de calidad</title>
    <style>
        body {
...
<body>
    <div class="content">
                ...
                <tbody>
                    <?php if (count($pedidos) > 0): ?>
                        <?php foreach ($pedidos as $pedido): ?>
                            <tr>
                                <td><?php echo htmlspecialchars($pedido['id_control_calidad']); ?></td>
                    ...
                    <?php else: ?>
                        <tr>
                            <td colspan="5">No se encontraron controles para el rango de fechas seleccionado.</td>
                        </tr>
                    <?php endif; ?>
                </tbody>
            </table>
        ...
        <?php if ($id_pedido && isset($_GET['act']) && $_GET['act'] == 'imprimir'): ?>
            <h3>Detalle de la Orden N° <?php echo htmlspecialchars($cabecera_compra['id_control_calidad']); ?></h3>
            <table>
                <tr>
                    <th>Código</th>
                    <td><?php echo htmlspecialchars($cabecera_compra['id_control_calidad']); ?></td>
                </tr>
                <tr>
                    <th>Sucursal</th>
            ...
            <table>
                ...
                <tbody>
                    <?php if (count($detalle_compra) > 0): ?>
                        <?php foreach ($detalle_compra as $detalle): ?>
                            <tr>
                                ...
                                <td><?php echo htmlspecialchars($detalle['estandar']); ?></td>
                    ...
                    <?php else: ?>
                        <tr>
                            <td colspan="5">No se encontraron detalles para el control de calidad seleccionado.</td>
                        </tr>
                    <?php endif; ?>
                </tbody>
            </table>
        ...
        <div class="footer-section">
            <p><strong>Generado por:</strong> Sistema de Gestión de control de calidad</p>
