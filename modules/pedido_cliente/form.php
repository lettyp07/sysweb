...
if ($_GET['form'] == 'add') { ?>
    <section class="content-header">
        <h1>
            <i class="fa fa-edit icon-title">Agregar Pedido cliente</i>
        </h1>
        <ol class="breadcrumb">
            <li><a href="?module=start"><i class="fa fa-home"></i>Inicio</a></li>
            <li><a href="?module=pedido_cliente"> Pedido Cliente</a></li>
        ...
        <div class="row">
            <div class="col-md-12">
                <div class="box box-primary">
                    <form role="form" class="form-horizontal" action="modules/pedido_cliente/proses.php?act=insert" method="POST">
                        <div class="box-body">
                            <?php
                            //Método para generar código
                            $query_id = mysqli_query($mysqli, "SELECT MAX(id_pedido_cliente) as id FROM pedido_cliente")
                            ...
                            <div class="form-group">
                                ...
                                <div class="col-sm-4">
                                    <select class="chosen-select" name="codigo_cliente" data-placeholder="Selecciona cliente"
                                            autocomplete="off" required id="codigo_cliente">
                                        <option value="0"></option>
                                        <?php
                                        $query_prov = mysqli_query($mysqli, "SELECT id_cliente, cli_nombre, cli_apellido, ci_ruc
                                        FROM clientes
                                        ORDER BY id_cliente ASC") or die('Error' . mysqli_error($mysqli));
                                        while ($data_prov = mysqli_fetch_assoc($query_prov)) {
                                            echo "<option value=\"$data_prov[id_cliente]\"> $data_prov[cli_nombre] | $data_prov[ci_ruc]</option>";
                                        }
                                        ?>
                                    </select>
                                </div>
                                ...
                                <div class="form-group">
                                    <div class="col-sm-offset-2 col-sm-10">
                                        <input type="submit" class="btn btn-primary btn-submit" name="Guardar"
                                            value="Guardar">
                                        <a href="?module=pedido_cliente" class="btn btn-default btn-reset">Cancelar</a>
                                    </div>
                                </div>
                            </div>
...
<script>
    ...
    function load(page) {
        ...
        $("#loader").fadeIn('slow');
        $.ajax({
            url: './ajax/productos_pedido_cliente.php',
            data: parametros,
...
<script>
    function agregar(id) {
        ...
        $.ajax({
            type: "POST",
            url: "./ajax/agregar_pedido_cliente.php",
            data: parametros,
            beforeSend: function (objeto) {
