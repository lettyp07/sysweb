...
$id_pedido = isset($_GET['id_pedido_cliente']) ? $_GET['id_pedido_cliente'] : '';
...
if ($id_pedido) {
    // Cabecera de compra
    $cabecera_compra_query = mysqli_query($mysqli, "SELECT pc.*, s.*,u.*
    FROM pedido_cliente pc
    JOIN usuarios u ON pc.id_user = u.id_user
    JOIN sucursal s ON u.id_sucursal = s.id_sucursal WHERE id_pedido_cliente = $id_pedido")
        or die('Error: ' . mysqli_error($mysqli));
    ...
    // Detalle de compra
    $detalle_compra_query = mysqli_query($mysqli, "SELECT dp.*, p.descrip, t.t_producto, u.u_descrip FROM detalle_pedido_cliente dp
    JOIN producto p
...
<body>
    <div class="content">
        ...
        <div class="header-section">
            <h2>Reporte de Pedido cliente</h2>
                ...
                <tbody>
                    <?php if (count($pedidos) > 0): ?>
                        <?php foreach ($pedidos as $pedido): ?>
                            <tr>
                                <td><?php echo htmlspecialchars($pedido['id_pedido_cliente']); ?></td>
        ...
        <?php if ($id_pedido && isset($_GET['act']) && $_GET['act'] == 'imprimir'): ?>
            <h3>Detalle del Pedido N° <?php echo htmlspecialchars($cabecera_compra['id_pedido_cliente']); ?></h3>
            <table>
                <tr>
                    <th>Código</th>
                    <td><?php echo htmlspecialchars($cabecera_compra['id_pedido_cliente']); ?></td>
                </tr>
                <tr>
                    <th>Sucursal</th>
                ...
                <tbody>
                    <?php if (count($detalle_compra) > 0): ?>
                        <?php foreach ($detalle_compra as $detalle): ?>
                            <tr>
                                <td><?php echo htmlspecialchars($detalle['id_pedido_cliente']); ?></td>
